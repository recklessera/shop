...
import { Mail, MessageCircle, Camera } from 'lucide-react'; // ADDED: Camera icon
...
export default function Footer({ settings }) {
  ...
  return (
    <> 
      {/* ADJUSTED: Reduced py-16 to py-12 for tighter fitting */}
      <footer className="bg-sidebar-bg text-gray-400 py-12 border-t border-sidebar-hover mt-auto font-sans">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          
          {/* Main Footer Grid - ADJUSTED: Tighter gap on smaller screens */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-10 mb-12">
            ...
            <div className="flex flex-col space-y-3 text-sm">
              ...
              <Link href="/shop" className="hover:text-brand-primary transition-colors w-max">Shop All</Link>
              <Link href="/collections" className="hover:text-brand-primary transition-colors w-max">Collections</Link>
              <Link href="/lookbook" className="hover:text-brand-primary transition-colors w-max">The Lookbook</Link>
              <Link href="/journal" className="hover:text-brand-primary transition-colors w-max">Journal</Link>
            </div>
            ...
            <div className="flex flex-col">
              ...
              {/* WhatsApp Channel Button - ADJUSTED: Shorter text, slightly smaller padding */}
              <a 
                href={whatsappChannelLink}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center justify-center bg-brand-primary text-black px-5 py-2.5 font-bold tracking-widest uppercase hover:bg-brand-accent hover:text-white transition-all duration-300 rounded-sm text-[11px] sm:text-xs w-max mb-8 group"
              >
                <svg 
                  viewBox="0 0 24 24" 
                  width="16" 
              ...
              <div className="flex space-x-6 items-center">
                ...
                {/* ADJUSTED: Replaced complex SVG with clean lucide-react Camera icon */}
      ...
      {/* Render the drawer outside the footer layout */}
      <PolicyDrawer 
        isOpen={isDrawerOpen} 
